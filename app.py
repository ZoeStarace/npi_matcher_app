...
def match_provider(row, state, limit, search_type):
    ...
    fuzzy_threshold = 70 
    specialty_fuzzy_threshold = 80  # You can adjust this threshold
    ...
    for i, (label, match_level) in enumerate(MATCH_STRATEGIES):
        ...
        if label == "Best: Full first and last name match":
            ...
            print(f"Found {len(matches)} matches for exact name: {first} {last}")
            ...
            # For fuzzy strategies, score and sort
            results_json = None
            ...
            matches = results_json.get('results', []) if results_json and results_json.get('result_count', 0) > 0 else []
            ...
            # Sort by specialty_score, then name_score, both descending
...
if uploaded_file:
    ...
    if error:
        ...
        def process_row(row):
            matches, row_for_results, specialty_matched = try_both_split_and_unsplit(row, state, limit, search_type)
            result_rows = []
            if matches:
